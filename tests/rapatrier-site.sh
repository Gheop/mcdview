...
#   GET  {base}/failures                     -> index JSON [{id,status,ext,...}]
#   GET  {base}/failures/<id>/diagnose.json  -> the --diagnose output
#   GET  {base}/failures/<id>/input.<ext>    -> the stored file
...
# one id per line, with its extension, from the index JSON
echo "$index" | python3 -c '
import json, sys
for e in json.load(sys.stdin):
    print(e["id"], e.get("ext", ""))
' | while read -r id ext; do
    ...
    curl -sfL -H "$auth" -o "$d/input$ext"    "$base/failures/$id/input$ext"    || ok=0
    if [ "$ok" = 1 ]; then
