...
# fixture, (converter tool, converter fn, expected tables, expected FKs)
CAS = [
...
def principal():
    ...
    for rel, outil, fonction, nt, nf in CAS:
        ...
        if fonction is None or not chemin.exists() or not shutil.which(outil):
            print(f'skip {rel} ({outil} absent)')
            continue
        sql = fonction(str(chemin))
        tables, fks = mcdview.analyser_sql(sql)
        faits += 1
        if len(tables) != nt:
